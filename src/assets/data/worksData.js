export const worksData = [
    {
        ...
        skill2: 'React',
        type: 'personal',
        desc: '서울 열린데이터 광장의 문화행사 REST API를 활용해 공연·전시·축제 정보를 탐색할 수 있는 웹 애플리케이션을 구현했습니다. TanStack Query를 사용해 데이터를 효율적으로 가져오고 캐싱했으며, 구/분류/요금 조건 필터와 키워드 검색, 정렬 기능을 결합해 다양한 방식으로 문화행사를 탐색할 수 있도록 설계했습니다. 검색어 입력 시 debounce를 적용해 불필요한 요청을 줄였고, 검색 결과에서 입력한 키워드를 하이라이트해 가독성을 높였습니다. 또한 Zustand 기반 즐겨찾기 상태 관리와 모달 상세보기, 카카오 지도 연동을 통해 실제 서비스에 가까운 사용자 경험을 구현했으며, GSAP을 활용해 카드 hover 인터랙션과 모달 전환 애니메이션을 적용해 UI 몰입도를 높였습니다.',
        details: {
            structure:
                '히어로, 검색창, 조건 필터, 카드 그리드, 페이지네이션, 상세 모달, 지도로 구성된 원페이지',
    ...
    },
    {
        ...
        skill2: 'React',
        type: 'personal',
        desc: 'Axios를 활용해 공공데이터 REST API를 호출하여 인천광역시 구/군별 약국 정보를 조회하는 웹 애플리케이션을 구현했습니다. 영업 시간 데이터를 기반으로 영업중, 곧 영업종료, 영업종료 상태를 실시간 계산해 UI에 반영했으며, 상태 필터와 더보기 페이지네이션을 통해 대용량 데이터를 효율적으로 탐색할 수 있도록 설계했습니다. 지도 검색은 주소와 약국명을 결합해 검색 정확도를 개선했으며, 영업 종료 시 전화 버튼 비활성화 등 상태 기반 UX를 적용해 실제 서비스에 가까운 사용자 경험을 구현했습니다. 또한 GSAP을 활용해 상태 요약 카운팅 애니메이션과 필터 슬라이딩 인터랙션 등을 구현하여 사용자 인터랙션의 몰입도를 높였습니다.',
        details: {
            structure:
                '히어로, 지역 선택, 상태 필터, 약국 리스트/카드 더보기 버튼등으로 구성된 원페이지',
            ratio: '디자인 100%, 코딩 100%',
        },
        tags: [
            'Figma',
    ...
    },
    {
        ...
        skill2: 'React',
        type: 'personal',
        desc: '공공데이터 REST API를 활용해 경기도 공공 도서관 정보를 조회하는 웹 애플리케이션을 구현했습니다. fetch를 사용해 데이터를 비동기로 가져오고, API 응답 데이터를 원본 상태로 유지한 뒤 지역 필터링, 검색, 정렬, 더보기 페이지네이션을 파생 상태로 분리해 처리했습니다. 검색 입력에는 debounce를 적용해 불필요한 연산을 줄였고, API 키를 환경변수로 관리해 보안을 고려했습니다. 또한 GSAP을 활용해 리스트 및 UI 요소에 자연스러운 등장 애니메이션을 적용해 사용자 경험을 개선했습니다.',
        details: {
            structure:
                '히어로, 검색/필터창, 도서관 리스트/카드, 더보기 버튼 등으로 구성된 원페이지',
            ratio: '디자인 100%, 코딩 100%',
        },
        tags: [
            'Figma',
